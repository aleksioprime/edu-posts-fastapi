...
Очистка удаляет обычных пользователей, все посты и их изображения, но сохраняет
суперпользователей, структуру таблиц и состояние миграций Alembic. Посты администратора
также удаляются. Операция необратима и требует явного флага `--yes`:

```bash
docker compose -p edu-posts exec api python scripts/clear_database.py --yes
```

Заполнение создаёт пользователей с воспроизводимыми логинами Faker и безопасными адресами
в домене `example.com`. При повторном запуске генерируются те же пользователи, поэтому
данные не дублируются. Заголовки и содержимое загружаются одним запросом из
[DummyJSON](https://dummyjson.com/docs/posts), а изображения-заглушки — из
[Placehold.co](https://placehold.co/). Для текста на каждом изображении Faker создаёт
короткое воспроизводимое название из двух случайных слов. Повторный запуск не дублирует
...
Если DummyJSON или Placehold.co отвечает ошибкой, скрипт выводит предупреждение и
автоматически создаёт локальные русские тексты и изображения-заглушки. Флаг
`--without-images` полностью отключает изображения, а `--local-text` сразу выбирает
встроенные русские тексты. Для полностью автономного запуска передайте оба флага:
...
Чтобы вместо автоматического fallback завершать команду при ошибке внешнего сервиса,
добавьте `--strict-external`.

Созданные логины выводятся после выполнения команды. Старые пользователи `demo1`,
`demo2`, … автоматически переименовываются с сохранением их постов. Администраторы при
очистке сохраняются, поэтому создавать суперпользователя заново не требуется. Не
используйте пароль тестовых пользователей для реальных аккаунтов.
